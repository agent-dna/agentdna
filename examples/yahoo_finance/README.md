...
- **Server** (`server.py`) is a pure remote (`enable_nft=False`) — it verifies the host envelope with `dna.handle(envelope)`, runs the `yfinance` call, then signs the reply with `dna.build(payload, ctx=ctx)`.
